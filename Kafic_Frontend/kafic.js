...
export class Kafic{
    constructor(id,naziv,adresa,kapacitet,maxljudi,maxlokala){
        ...
        this.stolovi=[];
        this.porudzbine=[];
        this.kontejner=null; //crtanje kafica
    ...
    crtajKafic(host){
        ...
        this.kontejner.classList.add("kontKafic");
        ...
        naslov.innerHTML=` Kafic ${this.naziv}`;
       ...
       var count=1;
        ...
        dugme.addEventListener("click",()=>
        {
           ...
           if(count>=this.maxlokala){
               btn.disabled=true;
               alert("Postoji maksimalan broj ovog kafica!")
    ...
    crtajFormu(host){
        ...
        kontForma1.appendChild(unos);
         ...
         kontForma2.className = "kontForma";   
          ...
          let deserti = ["","Kolaci", "Vocni kolac", "čokoladni hleb", "Palačinka Slatke", "Palačinka Slane", "Torta parče","Sladoled"];
          ...
          dugme.onclick=(ev)=>{
            ...
            fetch("https://localhost:5001/Kafic/OslobodiSto/"+ brojStola + "/"+ this.id,{
                method:"DELETE"
            }).then(resp=>{
                if(resp.ok){
        ...
        //dugmee za izmenu
         ...
         dugme.onclick=(ev)=>{
            ...
            fetch("https://localhost:5001/Kafic/IzmeniSto/"+brojStola + "/" + ime + "/" + prezime + "/" + brojLjudi,{
                method: "PUT"
            }).then(resp=>{
                if(resp.ok){
                   
                    this.stolovi[brojStola].izmeniSto(brojStola,brojLjudi,ime,prezime);
                    alert("Izmena je uspesna!");        
    ...
    crtajStolove(host){
        ...
        this.kontejner.querySelector(".divZaElemente").appendChild(stolovi);
            ...
            fetch("https://localhost:5001/Kafic/PreuzmiStolove1").then(p=>{
             p.json().then(data=>{
            var niz = [];
            var brel =0;
            data.forEach(sto=>{
